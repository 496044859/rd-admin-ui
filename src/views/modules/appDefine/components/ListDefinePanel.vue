<template>
  <section>
    <el-dialog title="列表定义" :visible.sync="defineDialogs.ListDefineDialog" width="750px" :close-on-click-modal="false">
      <div class="el-transfer">
        ...
        <div class="el-transfer-panel" style="width: 400px;">
          <el-table :data="defineNames" id="listDefine" row-key="id" @selection-change="handleDefineNamesSelectionChange" height="300" style="width: 100%">
            <el-table-column type="selection" width="55"></el-table-column>
            <el-table-column prop="mc" label="列表字段"></el-table-column>
            <el-table-column prop="dqfs" label="对齐">
              <template slot-scope="scope">
                <el-select v-model="scope.row.dqfs" placeholder="请选择" size="mini">
...
import { fetchAllNames, fetchDefineNames, save, applyDefine } from '@/api/appDefine/listDefine'
export default {
  ...
  watch: {
    'defineDialogs.ListDefineDialog': {
      handler (val) {
        if (val) {
          this.getAllNames()
          this.getDefineNames()
  ...
  computed: {
    ...
    moduleList () {
      return this.customList.filter(item => item.sflb === 1)
    }
  },
  methods: {
    getAllNames () {
    ...
    getDefineNames () {
      fetchDefineNames(this.customQuery).then(response => {
        ...
        this.oldList = this.defineNames.map(v => v.id)
    ...
    setSort () {
      const el = document.querySelectorAll('#listDefine .el-table__body-wrapper > table > tbody')[0]
