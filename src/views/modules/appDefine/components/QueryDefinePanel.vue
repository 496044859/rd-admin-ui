<template>
  <section>
    <el-dialog title="检索定义" :visible.sync="defineDialogs.QueryDefineDialog" width="550px" :close-on-click-modal="false">
      <div class="el-transfer">
        ...
        <div class="el-transfer-panel">
          <el-table :data="defineNames" id="queryDefine" row-key="id" @selection-change="handleDefineNamesSelectionChange" height="300" style="width: 100%">
            <el-table-column type="selection" width="55"></el-table-column>
            <el-table-column prop="mc" label="检索字段"></el-table-column>
          </el-table>
        </div>
      </div>
      <div slot="footer" class="dialog-footer">
        <el-button @click="defineDialogs.QueryDefineDialog = false">取 消</el-button>
...
import { fetchAllNames, fetchDefineNames, save, applyDefine } from '@/api/appDefine/queryDefine'
export default {
  name: 'QueryDefinePanel', // 检索定义
  props: {
    defineDialogs: {
      type: Object
    },
  ...
  watch: {
    'defineDialogs.QueryDefineDialog': {
      handler (val) {
        if (val) {
          this.getAllNames()
          this.getDefineNames()
  ...
  computed: {
    ...
    moduleList () {
      return this.customList.filter(item => item.sfjs === 1)
    }
  },
  methods: {
    getAllNames () {
    ...
    setSort () {
      const el = document.querySelectorAll('#queryDefine .el-table__body-wrapper > table > tbody')[0]
