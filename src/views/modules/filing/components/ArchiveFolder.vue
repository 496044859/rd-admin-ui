<template>
  <section>
    ...
    <serial-number :config="config" :colModel="colModel" :dialogVisible="dialogVisible" :multipleSelection="multipleSelection" @update="update"></serial-number>
...
import { fetchGridData, deletes, deptCheck, takeApartVerify, takeApart } from '@/api/folder'
...
export default {
  name: 'filing_ArchiveFolder', // 整理编目-案卷列表
  components: {
  ...
  methods: {
    ...
    handleFolderSonList (row) { // 进入卷内文件
      this.$router.push({
        path: '/collection/filing/folderFile',
        query: {
          ajid: row.id,
          dalxid: this.config.dalxid,
          kdlb: 'v',
