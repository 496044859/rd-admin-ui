<template>
  <section>
          ...
          <el-table :data="list" @selection-change="handleSelectionChange" v-loading="listLoading" stripe highlight-current-row style="width: 100%">
            ...
            <el-table-column label="操作">
              <template slot-scope="scope">
                <el-button size="mini" type="primary" @click="handleFolderSonList(scope.row)">卷内文件</el-button>
              </template>
            </el-table-column>
          </el-table>
        </div>
    ...
    <HandOver-UpLoad :handOverUpLoadQuery="handOverUpLoadQuery" :pcid="pcid"></HandOver-UpLoad>
  </section>
</template>

<script>
import { fetchGridData, handOver } from '@/api/folder'
...
export default {
  name: 'handOver_folderInside', // 档案移交-案卷层
  components: {
    GridColItem,
    // InternalTransfers
    HandOverUpLoad
  ...
  methods: {
    ...
    handleFolderSonList (row) {
      this.$router.push({
        path: '/archiveManagement/handOver/folderFileInside',
        query: {
          ajid: row.id,
          dalxid: this.config.dalxid,
          kdlb: 'f',
