<template>
  <section>
    <div class="toolbar">
      ...
      <el-button size="small" @click="handleBatchAdd" :disabled="currentNode.id==='0'">新增树节点</el-button>
      <el-button size="small" @click="handleDeletes" :disabled="this.multipleSelection.length===0&&currentNode.bs!==0">删除</el-button>
      <el-button size="small" @click="handleEditTree" v-if="currentNode.jdlx=='r'" :disabled="this.multipleSelection.length!==1&&currentNode.bs!==0">修改</el-button>
      <el-button size="small" @click="handleEditNode" v-else :disabled="this.multipleSelection.length!==1&&currentNode.bs!==0">修改</el-button>
    </div>
    ...
    <el-table :data="list" @selection-change="handleSelectionChange" v-loading.body="listLoading" highlight-current-row stripe style="width: 100%" id="ArchiveRangeType">
      ...
      <!-- <el-table-column label="分类类型">
        <template slot-scope="scope">
          <span>{{scope.row.sxmc | classFilter(propertyAllData)}}</span>
        </template>
      </el-table-column> -->
      <el-table-column prop="sxz" label="分类值">
        <template slot-scope="scope">
          <span>{{scope.row.flh | valueFilter}}</span>
        </template>
      </el-table-column>
    </el-table>
    ...
    <archive-tree-batch-add-nodes-form :rootFormQuery="rootFormQuery" :currentNode="currentNode" :formData="formData" @addData="addData" ></archive-tree-batch-add-nodes-form>
...
import { jzFetchTreeData, deletesJz } from '@/api/archiveTreeDefine/archiveTreeView'
...
export default {
  name: 'jzArchiveTreePropert', // 档案树属性
  components: {
  ...
  methods: {
    getList () {
      ...
      jzFetchTreeData({dasbm: this.currentNode.dasbm}).then(response => {
